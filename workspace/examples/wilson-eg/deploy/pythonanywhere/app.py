...
# Add PythonAnywhere (or other) origins via EXTRA_CORS_ORIGINS (comma-separated)
...
_default_db = 'sqlite:///' + os.path.join(INSTANCE_DIR, 'wilson.db').replace('\\', '/')
app.config['SQLALCHEMY_DATABASE_URI'] = os.environ.get('SQLALCHEMY_DATABASE_URI') or _default_db
...
UPLOAD_URL_PREFIX = 'http://127.0.0.1:5004/uploads/'
...
# --- SPA (frontend) serving: serve dist/ for non-API, non-uploads requests ---
@app.route('/')
def serve_spa_root():
    """Serve index.html for root (single-page app)."""
    if not os.path.isdir(DIST_DIR):
        return jsonify({'error': 'Frontend not built', 'message': 'Copy project/frontend/dist to deploy/pythonanywhere/dist'}), 503
    index_path = os.path.join(DIST_DIR, 'index.html')
    if not os.path.isfile(index_path):
        return jsonify({'error': 'index.html not found', 'message': 'Build frontend and copy to dist/'}), 503
    return send_from_directory(DIST_DIR, 'index.html')


@app.route('/<path:path>')
def serve_spa(path):
    """Serve static file from dist/ if it exists, else index.html (SPA fallback)."""
...
if __name__ == '__main__':
    db_path = os.path.join(INSTANCE_DIR, 'wilson.db')
