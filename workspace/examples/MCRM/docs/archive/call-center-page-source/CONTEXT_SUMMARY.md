...
| `escalated` | 3+ no_answer, supervisor queue |
...
```
no_answer #1 → wait 4h → retry
no_answer #2 → wait 4h → retry
no_answer #3 → order.status = 'escalated' → supervisor queue
```

`scheduled` and `left_message` do NOT count toward the 3-attempt limit.
