...
Note: All outcomes (including scheduled) increment the attempt counter per call session. Only `no_answer` triggers the 4h lockout. The 3-attempt escalation rule counts only `no_answer` attempts.
