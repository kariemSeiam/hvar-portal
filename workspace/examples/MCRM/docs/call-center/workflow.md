...
└─────────────────────────────────────────────────────────┘
           ↓ (confirmed only)
┌─────────────────────────────────────────────────────────┐
│  STEP 7 — ORDER AWAITS LEADER                           │
│                                                         │
│  POST /api/call-center/orders/<id>/confirm-by-customer  │
│                                                         │
│  Creates call record, stores confirmation_snapshot      │
│  Order status → 'confirmed' (no ticket yet)             │
│  Visible in "مؤكدة" tab                                 │
└─────────────────────────────────────────────────────────┘
           ↓
┌─────────────────────────────────────────────────────────┐
│  STEP 8 — LEADER APPROVES                               │
│                                                         │
│  POST /api/call-center/orders/<id>/leader-approve       │
│  (UI: "Process to Hub" / معالجة للنظام)                 │
│                                                         │
│  Creates service_ticket, order status → 'converted'     │
│  Leader: ✅ approve → ticket created, Hub workflow      │
│          ❌ reject  → back to agent (status=new)         │
└─────────────────────────────────────────────────────────┘
           ↓ (approved)
┌─────────────────────────────────────────────────────────┐
│  STEP 9 — HUB WORKFLOW                                  │
