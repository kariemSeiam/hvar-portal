/**
 * BostaContentPanel - Panel B: two full-width tabs (Bosta orders | Our tickets), or new customer form
 * Tab bar: single row, full horizontal width, two equal segments, underline active state.
 * FAB chips live in ServiceActionsPage sticky bar only.
 */
import React, { useState } from 'react';
import BostaOrdersGrid from './BostaOrdersGrid';
import BostaTicketsSection from './BostaTicketsSection';
import NewCustomerForm from './NewCustomerForm';

const TAB_ORDERS = 'orders';
const TAB_TICKETS = 'tickets';

/** Bosta logo SVG — same as ServiceActionsPage external pill */
function BostaLogoIcon({ className = 'h-5 w-auto', active = true }) {
    return (
        <svg width="112" height="32" viewBox="0 0 112 32" fill="none" xmlns="http://www.w3.org/2000/svg" className={`${className} text-brand-red-600 ${active ? 'opacity-100' : 'opacity-70'}`} aria-hidden>
...
export default function BostaContentPanel({
    customerData,
    customerOrders,
    existingServices,
    selectedOrder,
    ...
    if (customerData) {
        const ordersCount = customerOrders?.length ?? 0;
        const ticketsCount = Array.isArray(existingServices) ? existingServices.length : 0;

        return (
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col min-h-0 flex-1">
                {/* Tab bar — full width, two equal segments, underline active */}
                <div className="flex w-full border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50" dir="rtl" role="tablist" aria-label="طلبات وتذاكر">
                    <button
                        type="button"
                        role="tab"
                        onClick={() => setActiveTab(TAB_ORDERS)}
                        aria-selected={activeTab === TAB_ORDERS}
                        aria-controls="bosta-panel-orders"
                        id="tab-orders"
                        className={`flex-1 min-w-0 h-12 sm:h-14 flex items-center justify-center gap-2 font-cairo font-bold text-sm border-b-2 -mb-px transition-all duration-200 ${activeTab === TAB_ORDERS
                            ? 'text-brand-red-600 dark:text-brand-red-400 bg-white dark:bg-gray-800 border-brand-red-600 dark:border-brand-red-500'
                    ...
                    >
                        <BostaLogoIcon className="h-5 w-auto flex-shrink-0" active={activeTab === TAB_ORDERS} />
                        {ordersCount > 0 && <span className="tabular-nums text-inherit">({ordersCount})</span>}
                    </button>
                    <button
                        type="button"
                        role="tab"
                        onClick={() => setActiveTab(TAB_TICKETS)}
                        aria-selected={activeTab === TAB_TICKETS}
                        aria-controls="bosta-panel-tickets"
                        id="tab-tickets"
                        className={`flex-1 min-w-0 h-12 sm:h-14 flex items-center justify-center gap-1.5 font-cairo font-bold text-sm border-b-2 -mb-px transition-colors duration-200 ${activeTab === TAB_TICKETS
                            ? 'border-brand-blue-500 dark:border-brand-blue-400 text-brand-blue-600 dark:text-brand-blue-400 bg-white dark:bg-gray-800'
                    ...
                    >
                        ...
                        {ticketsCount > 0 && <span className="opacity-90 tabular-nums">({ticketsCount})</span>}
                    </button>
                </div>
                <div className="p-4 sm:p-5 flex flex-col min-h-0 flex-1 overflow-hidden" role="tabpanel" id={activeTab === TAB_ORDERS ? 'bosta-panel-orders' : 'bosta-panel-tickets'} aria-labelledby={activeTab === TAB_ORDERS ? 'tab-orders' : 'tab-tickets'}>
                    <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
                        ...
                        {activeTab === TAB_TICKETS && (
                            <BostaTicketsSection
                                tickets={existingServices ?? []}
                                onViewTicket={onViewTicket}
                                variant="full"
                                showTypeChips
                                emptyMessage="لا توجد تذاكر مرتبطة بهذا العميل أو بطلبات Bosta"
                            />
                        )}
                    </div>
                </div>
