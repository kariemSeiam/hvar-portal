...
function loadManifest() {
    const path = process.env.HUB_MCRM_MANIFEST_PATH ||
...
async function main() {
    ...
    server.registerResource("api-manifest", "hub-mcrm://manifest", {
    ...
    server.registerTool("hub_mcrm_api_call", {
        title: "Call HUB-MCRM API",
        description: "Call any HUB-MCRM API endpoint by operation_id. Use resource hub-mcrm://manifest to see all operation ids, methods, path/query/body params and error cases.",
        inputSchema: {
        ...
        if (operation_id === "erp_get_drafts") {
            if (!finalQuery.username && process.env.ERP_DEFAULT_USERNAME)
                finalQuery = { ...finalQuery, username: process.env.ERP_DEFAULT_USERNAME };
            if (!finalQuery.password && process.env.ERP_DEFAULT_PASSWORD)
                finalQuery = { ...finalQuery, password: process.env.ERP_DEFAULT_PASSWORD };
        }
