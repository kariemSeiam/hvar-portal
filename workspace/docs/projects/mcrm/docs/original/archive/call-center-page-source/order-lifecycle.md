...
escalated    → 3+ no_answer attempts; moved to supervisor queue
```
...
```
          ...
          ┌───────────────────┼───────────────────────────────┐
          ↓                   ↓                               ↓
      scheduled           confirmed                      (no_answer x3)
          │                   │                               │
          │                   ↓                               ↓
          │              converted                        escalated
          │               (→ ticket)
...
| `new` | `escalated` | 3rd no_answer attempt logged |
...
OUTCOME C: no_answer (up to 3 times)
    → order.attempt_count++
    → order.next_action_at = NOW() + 4h
    → (after 3rd) order.status = 'escalated'
...
| status | new/scheduled/confirmed/converted/canceled/escalated | PENDING/CONFIRMED/IN_PROCESS/READY_FOR_DISPATCH/SENT/COMPLETED/CANCELLED |
...
- `scheduled` and `left_message` do not count as attempts
- Reset is NOT automatic — escalated orders require supervisor action
