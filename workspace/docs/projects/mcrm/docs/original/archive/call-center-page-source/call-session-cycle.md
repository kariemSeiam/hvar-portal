...
│  OUTCOME BUTTONS                                                │
│  ─────────────────────────────────────────────────────          │
│  [ تأكيد ] [ جدولة ] [ لم يرد ] [ إلغاء ] [ رسالة ]            │
└─────────────────────────────────────────────────────────────────┘
```
...
Retry limit: 3 attempts
  ...
  - attempt 3 → order.status = 'escalated'
...
Agent left a voicemail or sent a WhatsApp message.

```
Triggered when: agent clicks [ رسالة ]

Required input: message notes

Backend actions:
  1. call record saved: calls.status = 'left_message'
  2. order remains in queue (status unchanged)
  3. Note visible on next agent who opens this order

Retry: order stays in queue, no lockout period.
```
...
status                ENUM('confirmed','scheduled','no_answer','canceled','left_message')
